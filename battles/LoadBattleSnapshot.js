...
import { normalizePveMatch } from "./PveEnemyCatalog.js";
...
export async function loadBattleSnapshot(matchId) {
    ...
    try {
        ...
        const normalizedPveMatch = normalizePveMatch(match);

        const snapshot = BattleSnapshot.create({
            matchId,
            seed: match.seed,
            terrain,
            units: normalizedPveMatch.units,
            mode: match.mode ?? "PVP",
            players: normalizedPveMatch.players,
            pve: normalizedPveMatch.pve
        });
